...
export async function createGoalRemote(goal: Omit<TrackedGoal, 'id'>): Promise<TrackedGoal> {
  const userId = await getCurrentUserId();
...
export async function updateGoalRemote(
  id: string,
  patch: Partial<TrackedGoal>
): Promise<TrackedGoal> {
  ...
  if (patch.scheduleEndMinutes !== undefined)
    payload.schedule_end_minutes = patch.scheduleEndMinutes;
